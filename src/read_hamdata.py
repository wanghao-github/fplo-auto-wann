...
class TBHamiltonian:
    ...
    def optimized_fplo_hop_block_to_wann_hr(self):
        ...
        for (index1, index2, block) in self.tij_hij_blocks:
            ...
            print("si_in_which_unitcell",si_in_which_unitcell)
            print("si_frac",si_frac)
            for row in block:
                r = np.array(row[:3])
                R = r + si
                R_frac = np.dot(inv_lattice_vectors, R)
                R_frac = np.round(R_frac, decimals=8)
                print("R_frac is ",R_frac)
                #T_int = np.floor(R_frac).astype(int) - si_in_which_unitcell
                #T_int = np.floor(R_frac).astype(int)
                T_int = np.round(R_frac).astype(int) - np.round(si_frac).astype(int)
                unique_r_vectors.append(T_int[:3])
                print("T_int is",T_int)
        # 确定唯一的 (r1, r2, r3) 组合
        np.savetxt('not_unique_r_vectors.txt', unique_r_vectors, fmt='%d')

        unique_r_vectors = np.unique(unique_r_vectors, axis=0)
        
        np.savetxt('unique_r_vectors.txt', unique_r_vectors, fmt='%d')
